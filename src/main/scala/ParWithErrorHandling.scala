...
object Par {
  ...
  def optionMap2[A,B,C](a: Option[A], b: Option[B])(f: (A, B) => C): Option[C] = {
  ...
  def map2[A,B,C](p: Par[A], p2:Par[B])(f: (A, B) => C): Par[C] =
    es => new Future[C] {
      def apply(cb: Option[C] => Unit): Unit = {
        ...
        val combiner = parallelism.ref.Actor[Either[Option[A],Option[B]]](es) {
            case Left(a) =>
              if (br.isDefined) eval(es)(cb(optionMap2(a, Some(br.get))(f)))
              else ar = a
            case Right(b) =>
              if (ar.isDefined) eval(es)(cb(optionMap2(Some(ar.get), b)(f)))
              else br = b
          }
  ...
  def map[A,B](p: Par[A])(f: A => B): Par[B] =
    es => new Future[B] {
      def apply(cb: Option[B] => Unit): Unit =
        p(es).apply((oa: Option[A]) =>  oa match {
          case Some(a) => 
            try {
              //val v = f(a)
              eval(es) { cb(Some(f(a))) }
  ...
  def eval[A](es: ExecutorService)(r: => Unit): Unit = {
    val f = es.submit(new Callable[Unit] { def call = r })
    try {
      f.get
    } catch {
      case t: Throwable => { println("Throw detected"); throw t }
    }
  }
  ...
  def run[A](es: ExecutorService)(p: Par[A]): Option[A] = {
    ...
    try {
      p(es).apply((a: Option[A]) => { ref.set(a); latch.countDown })
    } catch {
      case t: Throwable => { println("Throw seen in run");}
    } finally {
      latch.await
    }
